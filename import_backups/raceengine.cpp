...
extern uint8_t* psteer_cmd_idx_high_ghost;
extern uint8_t* psteer_cmd_idx_low_ghost;
...
uint8_t* psteer_cmd_idx_high = NULL;
uint8_t* psteer_cmd_idx_low = NULL;
float* pspeed = NULL;
...
ReOneStep(double deltaTimeIncrement)
{
	...
	if (pspeed == NULL){
		...
		psteer_cmd_idx_high = psteer_cmd_idx_high_ghost;
		psteer_cmd_idx_low = psteer_cmd_idx_low_ghost;
		pspeed = pspeed_ghost;
